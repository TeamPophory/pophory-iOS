...
SHARED_CONFIG_FILENAME="Shared.xcconfig"

# *.xconfig 파일의 전체 경로 계산
CONFIG_FILE_PATH="$FOLDER_PATH/$CONFIG_FILENAME"
SHARED_CONFIG_FILE_PATH="$FOLDER_PATH/$SHARED_CONFIG_FILENAME"

# 환경 변수에서 값을 가져와서 *.xconfig 파일에 추가하기
echo "BASE_URL = $BASE_URL" >> "$CONFIG_FILE_PATH"
echo "UNIT_AD_ID = $UNIT_AD_ID" >> "$CONFIG_FILE_PATH"
echo "SENTRY_DSN = $SENTRY_DSN" >> "$SHARED_CONFIG_FILE_PATH"
echo "ADMOB_APP_ID = $ADMOB_APP_ID" >> "$SHARED_CONFIG_FILE_PATH"
